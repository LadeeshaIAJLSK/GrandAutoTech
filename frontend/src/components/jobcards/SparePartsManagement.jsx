...
function SparePartsManagement({ jobCard, onUpdate, user }) {
  ...
  const canAdd = user.role.name === 'super_admin' || user.permissions.includes('add_job_card_spare_part')
  const canUpdate = user.role.name === 'super_admin' || user.permissions.includes('edit_job_card_spare_part')
  const canDelete = user.role.name === 'super_admin' || user.permissions.includes('delete_job_card_spare_part')
  const canApprove = user.permissions.includes('approve_job_card_spare_part') || user.role.name === 'super_admin'
  const canApproveSupervisor = user.role.name === 'super_admin' || user.permissions.includes('approve_job_card_spare_part_supervisor')
  const canApproveCustomer = user.role.name === 'super_admin' || user.permissions.includes('approve_job_card_spare_part_customer')
  const canConfirmDelivery = user.role.name === 'super_admin' || user.permissions.includes('confirm_job_card_spare_part_delivery')
  const canUpdateStatus = user.role.name === 'super_admin' || user.permissions.includes('update_job_card_spare_part_status')
  ...
  // Can mark as delivered if: has confirm_delivery permission AND (has update permission OR is assigned to task)
  const canMarkAsDelivered = (part) => {
    return canConfirmDelivery && (canUpdate || isAssignedToTask(part.task_id))
  }
  ...
  const handleAddPart = async (e) => {
    ...
    try {
      ...
      if (isEditingPart && selectedPart) {
        // Update existing part
        await axiosClient.put(`/spare-parts/${selectedPart.id}`, partForm, { headers: { Authorization: `Bearer ${token}` } })
        setNotification({ type: 'success', title: 'Success', message: 'Spare part updated successfully!' })
      } else {
        // Create new part
      ...
      setNotification({ type: 'error', title: 'Error', message: error.response?.data?.message || (isEditingPart ? 'Error updating part' : 'Error requesting part') })
    }
  }
  ...
  const handleStatusUpdate = async (partId, newStatus) => {
    if (!canUpdateStatus) {
      setNotification({ type: 'error', title: 'Unauthorized', message: 'You do not have permission to update spare part status' })
      return
    }
    try {
      const token = localStorage.getItem('token')
  ...
  const handleMarkAsDelivered = async (partId) => {
    if (!canConfirmDelivery) {
      setNotification({ type: 'error', title: 'Unauthorized', message: 'You do not have permission to confirm spare part delivery' })
      return
    }
    try {
      const token = localStorage.getItem('token')
      ...
      {parts.length === 0 ? (
        ...
        <div>
            ...
            <div className="mb-6 flex gap-3 items-center justify-between bg-gradient-to-r from-blue-50 to-indigo-50 p-5 rounded-xl border border-blue-200 flex-wrap">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-lg text-sm font-semibold transition-colors shadow-sm"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <button
                  ...
                  disabled={!canUpdateStatus}
                  className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-lg text-sm font-semibold transition-colors shadow-sm"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="space-y-3">
            {parts.map((part) => {
            const canApproveAdmin = (canApproveSupervisor) && part.admin_status === 'pending'
            const canApproveCustomerLevel = (canApproveCustomer) && part.customer_status === 'pending' && part.admin_status === 'approved'
            ...
            return (
              <div key={part.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4 p-3 bg-gray-50 rounded-lg">
                  <div>
                    <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Level 1 · Supervisor</p>
                ...
                {process.env.NODE_ENV === 'development' && (
                  <div className="text-xs text-gray-400 mb-3 font-mono bg-gray-50 px-2 py-1 rounded">
                    supervisor: {part.admin_status} · customer: {part.customer_status} · overall: {part.overall_status}
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-2 pt-3 border-t border-gray-100">
                  ...
                  {canUpdate && part.overall_status === 'pending' && (
                    <button onClick={() => openEditPartModal(part)}
                      className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-600 border border-blue-200 rounded-lg text-xs font-semibold transition-colors">
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <ModalHeader title={isEditingPart ? 'Edit Spare Part' : 'Request Spare Part'} subtitle={isEditingPart ? 'Update the part details below' : 'Fill in the part details below'} onClose={() => { setShowAddModal(false); setIsEditingPart(false); setPartForm({ task_id: '', part_name: '', part_number: '', quantity: '', description: '' }); }} />
            <form onSubmit={handleAddPart} className="px-7 py-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input type="number" value={partForm.quantity} onChange={(e) => setPartForm({...partForm, quantity: e.target.value})} placeholder="e.g., 4" min="0"
                    className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-lg bg-gray-50 focus:bg-white focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/10 transition-all" />
                </div>
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex justify-end gap-3 pt-5 border-t border-gray-100">
                <button type="button" onClick={() => { setShowAddModal(false); setIsEditingPart(false); setPartForm({ task_id: '', part_name: '', part_number: '', quantity: '', description: '' }); }} className="px-5 py-2.5 text-sm bg-white hover:bg-gray-50 text-gray-700 rounded-lg font-semibold border border-gray-300 shadow-sm transition-colors">Cancel</button>
                <button type="submit" className="px-5 py-2.5 text-sm bg-primary hover:bg-primary-dark text-white rounded-lg font-bold transition-all shadow-md hover:shadow-lg hover:-translate-y-px" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.2)' }}>{isEditingPart ? 'Update Part' : 'Request Part'}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
            ...
            <div className="flex justify-end gap-3 px-7 py-4 border-t border-gray-100">
              ...
              <button onClick={handleMarkAsOrdered} className="inline-flex items-center gap-1.5 px-5 py-2 text-sm bg-blue-500 hover:bg-blue-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-lg font-bold transition-colors shadow-sm" disabled={!canUpdateStatus}>
              ...
              <button onClick={handleHaveIt} className="inline-flex items-center gap-1.5 px-5 py-2 text-sm bg-green-500 hover:bg-green-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-lg font-bold transition-colors shadow-sm" disabled={!canUpdateStatus}>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
            ...
            <div className="flex justify-end gap-3 px-7 py-4 border-t border-gray-100">
              ...
              <button onClick={handleMarkAsReceived} disabled={!canUpdateStatus} className="inline-flex items-center gap-1.5 px-5 py-2 text-sm bg-green-500 hover:bg-green-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white rounded-lg font-bold transition-colors shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                Mark as Received
              </button>
            </div>
