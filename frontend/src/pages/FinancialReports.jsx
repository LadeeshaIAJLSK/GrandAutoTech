  ...
  const fetchReports = async () => {
    try {
      ...
      const [financial, methods, dues] = await Promise.all([
        axiosClient.get('/reports/financial-summary', { params: { start_date: startDate, end_date: endDate }, headers: { Authorization: `Bearer ${token}` } }),
        axiosClient.get('/reports/payment-methods', { params: { start_date: startDate, end_date: endDate }, headers: { Authorization: `Bearer ${token}` } }),
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        ...
        <div className="flex gap-4">
          <div className="space-y-1.5">
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide">Start Date</label>
            <input
              type="date"
              value={startDate}
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide">End Date</label>
            <input
              type="date"
              value={endDate}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-green-600 mt-0.5">{formatCurrency(financialData?.total_revenue || 0)}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Outstanding Dues</p>
            <p className="text-xl font-bold text-red-500 mt-0.5">{formatCurrency(outstandingDues?.total_outstanding || 0)}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Paid Job Cards</p>
            <p className="text-xl font-bold text-blue-600 mt-0.5">{financialData?.paid_job_cards || 0}</p>
          </div>
        </div>
      </div>
